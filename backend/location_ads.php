...
<head>
    ...
    <title> Stuffs From <?php echo $fata['location']; ?> | myPropertyDeets</title>
...
<body>
    ...
    <div class="page-content-wrapper d-flex flex-column" style="margin: 20px">
        <?php see_status($_REQUEST); ?>
            ...
            <h4 style="color: #fb484e">Stuff Available in <?php echo $fata['location']; ?>   <a href="stuff_available.php" style="float: right;" class="btn btn-primary btn-icon-right">View all
            ...
            <div class="tabs">
                <ul class="nav nav-pills tab-style-01 font-size-lg" role="tablist">
                    <li class="nav-item"> <a class="nav-link active" id="all-tab" data-toggle="tab" href="#all" role="tab" aria-controls="all" aria-selected="true">All Listings 
                        <?php try {
                                $stmt = $pdo->prepare('SELECT * FROM `my_stuff` WHERE `uploaded_by_id` <> "'.$pdo_auth['id'].'"  AND `location` LIKE "%'.$fata['location'].'%" AND `status`="Active"  ORDER BY date DESC');
                            ...
                            echo $count; 
                            ?> </a></li>
                    <li class="nav-item"> <a class="nav-link" id="active-tab" data-toggle="tab" href="#active" role="tab" aria-controls="active" aria-selected="false">For Sale 
                         <?php try {
                                $stmt = $pdo->prepare('SELECT * FROM `my_stuff` WHERE `uploaded_by_id` <> "'.$pdo_auth['id'].'"  AND `location` LIKE "%'.$fata['location'].'%" AND `status`="Active" AND `type`="Sell"  ORDER BY date DESC');
                            } catch(PDOException $ex) {
                                echo "An Error occured!"; 
                                print_r($ex->getMessage());
                            }
                    ...
                    <li class="nav-item"> <a class="nav-link" id="rent-tab" data-toggle="tab" href="#rent" role="tab" aria-controls="rent" aria-selected="false">For Rent 
                        <?php try {
                                $stmt = $pdo->prepare('SELECT * FROM `my_stuff` WHERE `uploaded_by_id` <> "'.$pdo_auth['id'].'"  AND `location` LIKE "%'.$fata['location'].'%" AND `status`="Active" AND `type`="Rent"  ORDER BY date DESC');
                            } catch(PDOException $ex) {
                                echo "An Error occured!"; 
                                print_r($ex->getMessage());
                            }
