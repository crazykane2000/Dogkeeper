                                        ...
                                        <?php see_status($_REQUEST); ?>
                                           <div style="padding: 20px;background-color: #fff;width:100%;min-height: 600px">
                                                ...
                                                <table  style="color: #333;font-size: 13px;" class="table table-striped table-hover">
                                                  ...
                                                  <?php 
                                                  try {
                                                        $stmt = $pdo->prepare('SELECT * FROM `faq`   ORDER BY date DESC');
                                                    ...
                                                    foreach($user as $key=>$value){                                 
                                                      echo '<tr>
                                                            <td>'.$i.'</td>
                                                            <td><b>'.$value['question'].'</b><br/>'.newline($value['answer'],15).'</td>
                                                            <td>'.$value['date'].'</td>      
                                                            <th><a href="delete_faq.php?id='.$value['id'].'" onclick="return confirm(\' Are You Sure You need to Delete this? \');"><button class="btn btn-danger btn-sm">Delete</button></a>  
                                                            <a href="edit_faq.php?id='.$value['id'].'" ><button class="btn btn-primary btn-sm">Edit</button></a>  
                                                            </th>                        
                                                        </tr>';
                                                        $i++;
                                                    }           
